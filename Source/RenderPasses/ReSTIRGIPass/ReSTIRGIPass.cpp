...
bool ReSTIRGIPass::parseDictionary(const Dictionary& dict)
{
...
Dictionary ReSTIRGIPass::getSpecializedScriptingDictionary()
{
    Dictionary d;
    d[kMaxSurfaceBounces] = mStaticParams.maxSurfaceBounces;
    d[kSpatialMisKind] = mStaticParams.spatialMisKind;
...
Texture::SharedPtr ReSTIRGIPass::createNeighborOffsetTexture(uint32_t sampleCount)
{
...
bool ReSTIRGIPass::renderStatsUI(Gui::Widgets& widget)
{
    bool dirty = false;
...
void ReSTIRGIPass::tracePass(RenderContext* pRenderContext, const RenderData& renderData, const ComputePass::SharedPtr& pass, const std::string& passName, int sampleID)
{
    PROFILE(passName);
